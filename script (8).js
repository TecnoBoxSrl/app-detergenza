...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function filtraTabella() {
        ...
        document.querySelectorAll('#tabella-prodotti tbody tr').forEach(tr => {
            const codArticolo = tr.children[0].textContent.toLowerCase();
            const prodotto = tr.children[1].textContent.toLowerCase();
            const superfici = tr.children[2].textContent.toLowerCase();
            const descrizione = tr.children[3].textContent.toLowerCase();
            const testoTotale = codArticolo + ' ' + prodotto + ' ' + superfici + ' ' + descrizione;
            const visibile = testoTotale.includes(testoProdotto) && superfici.includes(testoSuperfici);
