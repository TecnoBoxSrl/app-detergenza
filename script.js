...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const testoRiga = Array.from(tr.children).map((td, idx) => {
        if (idx === 9 || idx === 10 || idx === 11) return ''; // ignora schede tecniche, sicurezza e immagini
        return td.textContent;
                            ...
                            Array.from(tr.children).forEach((td, index) => {
                                if (index === 9 || index === 10 || index === 11) {
                                    return; // salta evidenziazione su link e immagini
                                }
